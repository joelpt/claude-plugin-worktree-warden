...
class ImportFailureTest(unittest.TestCase):
    ...
    def _env_broken(self) -> dict[str, str]:
        """Return an environment with CLAUDE_PLUGIN_ROOT pointing at an empty scripts dir."""
        env = {**os.environ}
    ...
    def test_enforce_hook_import_failure_exits_0(self) -> None:
        payload: dict[str, object] = {"tool_name": "Edit", "cwd": str(self.repo), "tool_input": {"file_path": str(self.repo / "x.py")}}
    ...
    def test_guard_hook_import_failure_exits_0(self) -> None:
        payload: dict[str, object] = {"tool_name": "Bash", "cwd": str(self.repo), "tool_input": {"command": "git worktree remove --force /tmp/wt"}}
