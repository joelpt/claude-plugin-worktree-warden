...
- **F-A (correctness, Phase-1, highest) — DONE (both options shipped):** the multi-process merge holds a *lease* that can lapse during a long HITL/test pause; a 2nd session could then `acquire-main` and two merges race to `main`. **Option A (narrow the window):** operation locks (merge/bumpall) carry a 120-min lease (`MERGE_LEASE_SECONDS`, configurable `merge_lease_seconds`), renewed at that length by both `acquire` and the engine's per-subcommand `refresh`, so a realistic pause no longer lapses it; the bound is documented honestly (mutual exclusion holds only within the window; `force-unlock` is the escape). **Option B (close the window) — shipped:** the engine now *aborts* a mutating step whose `refresh` returns `"lost"` when invoked with `--require-lease` (new `EXIT_LEASE_LOST=19`), so a reclaimed/force-unlocked key halts the merge **before** any mutation instead of racing a second writer. The flag is opt-in (only the orchestrator knows it holds a lease — a bare CLI `land` that never acquired is unaffected; `refresh` alone can't tell "lost it" from "never had it"); `undo` deliberately has **no** such flag, so recovery is never blockable by the lease-loss it recovers from. The abort surfaces `details.holder` (the live reclaimer, or `null` after a force-unlock) so the merge skill knows whether `undo` is safe or would collide with a live writer, and logs an `lease-lost` audit event. The merge skill passes `--require-lease` on snapshot/land/rebase-continue/teardown and treats `19` as a global hard-halt. **Orthogonal to occupancy** (primary-key, decoupled).
