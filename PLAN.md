...
- **F-A (correctness, Phase-1, highest) — half done:** the multi-process merge holds a *lease* that can lapse during a long HITL/test pause; the engine's per-subcommand `refresh()` returns `"lost"` but currently logs-and-continues, so a 2nd session could `acquire-main` and two merges race to `main`. **A is now applied** — operation locks (merge/bumpall) carry a 120-min lease (`MERGE_LEASE_SECONDS`, configurable `merge_lease_seconds`), renewed at that length by both `acquire` and the engine's per-subcommand `refresh`, so a realistic pause no longer lapses it; the lease-window bound is documented honestly (mutual exclusion holds only within the window; `force-unlock` is the escape). **B remains a TDD'd follow-up:** make the engine *abort* on a `"lost"` refresh (exit non-zero; merge skill halts / undo), which requires the mid-merge undo path read+tested first. Pre-existing on `main`; **orthogonal to occupancy** (primary-key, decoupled).
